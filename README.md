...
ถ้าต้องการกรอกค่าผ่านฟอร์มแทนการพิมพ์ flag สามารถเปิดหน้า GUI local ได้:

```powershell
.\mariadb-installer.exe --gui
```

จะเปิด browser ที่ `http://127.0.0.1:8079` โดยอัตโนมัติ

จุดที่กรอกได้ในหน้า GUI:

- host / SSH port / user
- auth แบบ password หรือ key
- password / sudo password
- charset / verbose / skip cleanup
- config JSON สำหรับหลายเครื่องหรือ Galera cluster
- save / load / delete profile เพื่อเก็บค่าที่ใช้บ่อย

ถ้าต้องการหน้าต่างแบบ native Windows แทน browser:

```powershell
.\mariadb-installer.exe --desktop-gui
```
...
# SSH เป็น user อื่นแล้วให้โปรแกรม sudo ต่อ (ถ้า sudo ต้องใช้รหัสผ่าน)
...
- ถ้า SSH login ด้วย user อื่นที่ไม่ใช่ `root` โปรแกรมจะยกระดับคำสั่งผ่าน `sudo` อัตโนมัติ;
  ถ้าเครื่องปลายทางต้องใส่รหัสผ่านตอน `sudo` ให้เพิ่ม `"sudo_password"` หรือ `--sudo-password`
  ตามตัวอย่างข้างบน
- ถ้าไม่ต้องการใช้ sudo password และเครื่องปลายทางอนุญาต `sudo -n` แบบไม่ถามรหัสผ่าน
  ให้ปล่อย `"sudo_password"` ว่างได้
...
| `--gui`          | เปิดหน้า GUI local ใน browser สำหรับกรอกค่าได้ง่ายขึ้น             |
| `--gui-port`     | port ของ GUI local (default `8079`)                                |
| `--desktop-gui`  | เปิด native Windows desktop GUI                                    |
...
| `--sudo-password` | password สำหรับ `sudo` เมื่อ login ด้วย user อื่นที่ไม่ใช่ `root` |
...
- ถ้า SSH login ด้วย user อื่นที่ไม่ใช่ `root` โปรแกรมจะยกระดับคำสั่งผ่าน `sudo` ให้เอง;
  ถ้าเครื่องปลายทางต้องใช้รหัสผ่านตอน `sudo` ให้ส่ง `--sudo-password` เพิ่ม
- โปรไฟล์ที่บันทึกจาก GUI จะเก็บค่าคอนฟิกที่ใช้บ่อย แต่จะไม่เก็บ SSH password / sudo password
  เพื่อไม่ให้เผลอเขียนรหัสผ่านลงดิสก์เป็น plain text
